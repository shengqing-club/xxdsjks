  ...
  try {
    ...
    const result = await pool.query(
      `SELECT id, file_name, original_name, file_size, file_type,
       ...
       FROM group_files WHERE group_id = ? ORDER BY created_at DESC`,
      [groupId]
    )
...
router.post('/:group_id/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    const newRow = await pool.query('SELECT * FROM group_files WHERE id = ?', [insertedId])
...
router.get('/download/:file_id', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM group_files WHERE id = ?', [req.params.file_id])
...
// 分片下载分组文件（绕过 Netlify 6MB 限制）
router.get('/download/:file_id/chunk', createChunkedDownloadHandler({
