...
router.get('/upcoming', async (req, res) => {
  try {
    ...
    let sql = `
      SELECT id, course_name, exam_date, exam_time, duration, location, class_name, description, created_at, updated_at FROM exams
...
router.get('/class/:className', async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT id, course_name, exam_date, exam_time, duration, location, class_name, description, created_at, updated_at FROM exams WHERE class_name = ? ORDER BY exam_date ASC, exam_time ASC`,
...
router.get('/', adminMiddleware, async (req, res) => {
  try {
    ...
    let sql = 'SELECT id, course_name, exam_date, exam_time, duration, location, class_name, description, created_at, updated_at FROM exams WHERE 1=1'
    const params = []
    let idx = 1
    if (class_name) {
      sql += ` AND class_name = ?`
  ...
  try {
    ...
    for (const cls of classes) {
      ...
      const newRow = await pool.query(
        'SELECT id, course_name, exam_date, exam_time, duration, location, class_name, description, created_at, updated_at FROM exams WHERE id = ?',
        [insertedId]
      )
