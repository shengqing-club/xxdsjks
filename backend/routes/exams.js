...
router.get('/upcoming', async (req, res) => {
  try {
    ...
    let sql = `
      SELECT id, course_name, exam_date::text as exam_date, exam_time, duration, location, class_name, description, created_at, updated_at FROM exams
      WHERE exam_date >= CURRENT_DATE
    `
    const params = []
    if (class_name) {
      sql += ' AND class_name = $1'
...
router.get('/class/:className', async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT id, course_name, exam_date::text as exam_date, exam_time, duration, location, class_name, description, created_at, updated_at FROM exams WHERE class_name = $1 ORDER BY exam_date ASC, exam_time ASC`,
...
router.get('/', adminMiddleware, async (req, res) => {
  try {
    ...
    let sql = 'SELECT id, course_name, exam_date::text as exam_date, exam_time, duration, location, class_name, description, created_at, updated_at FROM exams WHERE 1=1'
    const params = []
    let idx = 1
    if (class_name) {
    ...
    for (const cls of classes) {
      const result = await pool.query(
        `INSERT INTO exams (course_name, exam_date, exam_time, duration, location, class_name, description)
         VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id, course_name, exam_date::text as exam_date, exam_time, duration, location, class_name, description, created_at, updated_at`,
...
router.put('/:id', adminMiddleware, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE exams SET course_name=$1, exam_date=$2, exam_time=$3, duration=$4, location=$5,
       class_name=$6, description=$7, updated_at=CURRENT_TIMESTAMP WHERE id=$8 RETURNING id, course_name, exam_date::text as exam_date, exam_time, duration, location, class_name, description, created_at, updated_at`,
