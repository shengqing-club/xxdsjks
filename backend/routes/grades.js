...
router.get('/student/:studentId', async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM grades WHERE student_id = ? ORDER BY semester DESC, course_name',
...
// 课程类型成绩统计（管理员）
router.get('/stats/course-type', adminMiddleware, async (req, res) => {
  try {
    const result = await pool.query(`
...
router.post('/', adminMiddleware, async (req, res) => {
  try {
    ...
    const newRow = await pool.query('SELECT * FROM grades WHERE id = ?', [insertedId])
...
router.put('/:id', adminMiddleware, async (req, res) => {
  try {
    const { studentId, courseName, score, semester, courseType } = req.body
    await pool.query(
      `UPDATE grades SET student_id=?, course_name=?, score=?, course_type=?, semester=?, updated_at=CURRENT_TIMESTAMP
       WHERE id=?`,
      [studentId, courseName, score, courseType, semester, req.params.id]
    )
    const result = await pool.query('SELECT * FROM grades WHERE id = ?', [req.params.id])
...
router.delete('/:id', adminMiddleware, async (req, res) => {
  try {
    ...
    if (result.affectedRows === 0) return res.status(404).json({ message: '成绩记录不存在' })
