...
router.get('/posts', async (req, res) => {
  try {
    ...
    const isLikedSql = userId
      ? `(SELECT COUNT(*) FROM forum_likes WHERE post_id = p.id AND user_id = ?) as is_liked`
      : '0 as is_liked'
...
router.post('/posts', async (req, res) => {
  try {
    ...
    const newRow = await pool.query('SELECT * FROM forum_posts WHERE id = ?', [insertedId])
...
router.delete('/posts/:id', async (req, res) => {
  try {
    const userId = req.user.studentId || req.user.username
    const post = await pool.query('SELECT author_id FROM forum_posts WHERE id = ?', [req.params.id])
    ...
    await pool.query('DELETE FROM forum_comments WHERE post_id = ?', [req.params.id])
    await pool.query('DELETE FROM forum_likes WHERE post_id = ?', [req.params.id])
...
router.put('/posts/:id/pin', adminMiddleware, async (req, res) => {
  try {
    ...
    await pool.query('UPDATE forum_posts SET pinned = ? WHERE id = ?', [pinned ? 1 : 0, req.params.id])
    const result = await pool.query('SELECT * FROM forum_posts WHERE id = ?', [req.params.id])
    if (result.rows.length === 0) return res.status(404).json({ message: '帖子不存在' })
    res.json(result.rows[0])
  } catch (e) {
...
router.post('/posts/:id/comments', async (req, res) => {
  try {
    ...
    const insertedId = result.insertId
    const newRow = await pool.query('SELECT * FROM forum_comments WHERE id = ?', [insertedId])
    res.status(201).json(newRow.rows[0])
  } catch (e) {
...
router.delete('/comments/:id', async (req, res) => {
  try {
    const userId = req.user.studentId || req.user.username
    const comment = await pool.query('SELECT author_id FROM forum_comments WHERE id = ?', [req.params.id])
    ...
    await pool.query('DELETE FROM forum_comments WHERE id = ?', [req.params.id])
    res.json({ message: '删除成功' })
  } catch (e) {
    res.status(500).json({ message: '删除失败' })
  }
...
router.post('/posts/:id/like', async (req, res) => {
  try {
    ...
    if (existing.rows.length > 0) {
      await pool.query('DELETE FROM forum_likes WHERE post_id = ? AND user_id = ?', [req.params.id, userId])
...
router.get('/posts/:id/likes', async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT user_id, user_name FROM forum_likes WHERE post_id = ? ORDER BY created_at DESC',
      [req.params.id]
    )
    res.json(result.rows)
  } catch (e) {
