...
router.put('/change-password', authMiddleware, async (req, res) => {
  try {
    ...
    if (role === 'admin') {
      const result = await pool.query('SELECT password_hash FROM admins WHERE id = $1', [userId])
      ...
      const result = await pool.query('SELECT password_hash FROM students WHERE id = $1', [userId])
      if (result.rows.length === 0) return res.status(404).json({ message: '用户不存在' })
      const valid = await bcrypt.compare(oldPassword, result.rows[0].password_hash)
      if (!valid) return res.status(401).json({ message: '旧密码错误' })
      const newHash = await bcrypt.hash(newPassword, 10)
      await pool.query('UPDATE students SET password_hash = $1 WHERE id = $2', [newHash, userId])
    } else {
