...
router.post('/', adminMiddleware, async (req, res) => {
  try {
    ...
    const newRow = await pool.query('SELECT * FROM groups WHERE id = ?', [insertedId])
...
router.get('/:id', async (req, res) => {
  try {
    const groupResult = await pool.query('SELECT * FROM groups WHERE id = ?', [req.params.id])
...
router.delete('/:id', adminMiddleware, async (req, res) => {
  ...
  try {
    ...
    // 1. 标记分组为解散状态
    await client.query(
      'UPDATE groups SET is_active = 0, disbanded_at = CURRENT_TIMESTAMP WHERE id = ?',
      [groupId]
    )

    // 2. 清空分组聊天消息
    await client.query('DELETE FROM group_chat_messages WHERE group_id = ?', [groupId])

    // 3. 清空分组文件
    await client.query('DELETE FROM group_files WHERE group_id = ?', [groupId])

    // 4. 移除分组成员
    await client.query('DELETE FROM group_members WHERE group_id = ?', [groupId])
...
router.post('/:id/members', async (req, res) => {
  try {
    ...
    // MySQL: 使用 INSERT IGNORE 替代 ON CONFLICT
    const result = await pool.query(
      `INSERT IGNORE INTO group_members (group_id, student_id, student_name, role)
       VALUES (?,?,?,?)`,
...
router.post('/:id/leader/:student_id', adminMiddleware, async (req, res) => {
  ...
  try {
    ...
    // 取消现有组长
    await client.query(
      "UPDATE group_members SET role = 'member' WHERE group_id = ? AND role = 'leader'",
      [groupId]
    )

    // 设置新组长
    await client.query(
      "UPDATE group_members SET role = 'leader' WHERE group_id = ? AND student_id = ?",
      [groupId, studentId]
    )
