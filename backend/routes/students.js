...
router.get('/', adminMiddleware, async (req, res) => {
  try {
    ...
    const params = []
    if (keyword) {
      sql += ` AND (name ILIKE $${params.length + 1} OR student_id ILIKE $${params.length + 1} OR major ILIKE $${params.length + 1})`
      params.push(`%${keyword}%`)
    }
    if (major) {
      sql += ` AND major = $${params.length + 1}`
      params.push(major)
    }
    if (status) {
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      try {
        ...
        const existing = await pool.query('SELECT id FROM students WHERE student_id = $1', [studentId])
        if (existing.rows.length > 0) {
          await pool.query(
            'UPDATE students SET name=$1, gender=$2, age=$3, major=$4, class_name=$5, status=$6, password_hash=$7, updated_at=CURRENT_TIMESTAMP WHERE student_id=$8',
            [name, gender, age, major, className, status, password, studentId]
          )
          success.push({ studentId, name, action: '更新' })
        } else {
          await pool.query(
            'INSERT INTO students (student_id, name, gender, age, major, class_name, status, password_hash) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)',
