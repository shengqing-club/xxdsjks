...
router.get('/', async (req, res) => {
  try {
    ...
    if (isAdmin) {
      ...
      sql = 'SELECT * FROM rewards_punishments WHERE student_id = ? ORDER BY created_at DESC'
      params = [userId]
    }
...
router.post('/', adminMiddleware, async (req, res) => {
  try {
    ...
    const newRow = await pool.query('SELECT * FROM rewards_punishments WHERE id = ?', [insertedId])
...
router.delete('/:id', adminMiddleware, async (req, res) => {
  try {
    await pool.query('DELETE FROM rewards_punishments WHERE id = ?', [req.params.id])
