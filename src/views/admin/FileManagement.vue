    ...
    <el-card shadow="never" class="toolbar-card">
      <div class="toolbar">
        ...
        <div class="toolbar-right">
          ...
          <span class="toggle-label">学生上传权限：</span>
          <el-switch
            v-model="studentUploadEnabled"
            active-text="已开启"
            inactive-text="已关闭"
            @change="toggleStudentUpload"
          />
        </div>
        ...
        <el-table-column label="上传者" width="120">
          <template #default="{ row }">
            <el-tag :type="row.uploaderRole === 'admin' ? '' : 'success'" size="small" effect="plain">
              {{ row.uploaderName || (row.uploaderRole === 'admin' ? '管理员' : '学生') }}
            </el-tag>
          </template>
        </el-table-column>
...
const toggleStudentUpload = async (val) => {
  try {
    await setStudentUploadSetting(val)
...
// Quick upload (from toolbar button)
...
const handleUpload = () => {} // placeholder for el-upload
...
.toggle-label { font-size: 13px; color: #475569; white-space: nowrap; }
.result-count { font-size: 14px; color: #64748b; margin-left: 12px; }
